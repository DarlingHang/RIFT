...
PhaseCongruency::PhaseCongruency(Size _size, size_t _nscale, size_t _norient)
{
    ...
    eo = std::vector<std::vector<Mat> >(nscale,vector<Mat>(norient)); //s*o 
    ...
    const int dft_M = _size.height;
    const int dft_N = _size.width;
    ...
    for (int row = dft_M_2 - r; row < dft_M_2 + r; row++)
    {
        ...
        {
            double m = static_cast<double>((row - dft_M_2)) / dft_M;
            double n = static_cast<double>((col - dft_N_2)) / dft_N;
...
void PhaseCongruency::calc(InputArray _src, std::vector<cv::Mat> &_pc)
{
    ...
    const int dft_M_r = src.rows - src.rows;
    const int dft_N_c = src.cols - src.cols;
    ...
    for (unsigned o = 0; o < norient; o++)
    {
        ...
        for (unsigned scale = 0; scale < nscale; scale++)
        {
            ...
            filtered(Rect(0, 0, width, height)).copyTo(eo[scale][o]);
...
void PhaseCongruency::feature(std::vector<cv::Mat>& _pc, cv::OutputArray _edges, cv::OutputArray _corners)
{
    ...
    divide(minMoment, 2, minMoment);
    divide(maxMoment, 2, maxMoment);
    
    double minv, maxv;
	Point pt_min, pt_max;
    normalize(maxMoment, maxMoment, 0.0, 1.0, NORM_MINMAX);
	minMaxLoc(maxMoment, &minv, &maxv, &pt_min, &pt_max);
